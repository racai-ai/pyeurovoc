...
def load_data(data_path, batch_size):
    ...
    for directory in os.listdir(data_path):
        ...
        train_y = torch.from_numpy(np.load(os.path.join(data_path, directory, "train_y.npy")))
        ...
        dev_y = torch.from_numpy(np.load(os.path.join(data_path, directory, "dev_y.npy")))
        ...
        test_y = torch.from_numpy(np.load(os.path.join(data_path, directory, "test_y.npy")))
        ...
        dev_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=False)

        dataset = torch.utils.data.TensorDataset(test_X, test_mask, test_y)
        test_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=False)

        dict_datasets[directory[:2]] = (train_loader, dev_loader, test_loader)
